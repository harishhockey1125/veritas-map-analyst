    ...
    <div className="bg-slate-800/50 backdrop-blur-sm border-r border-slate-700 h-full p-6 flex flex-col gap-8 overflow-y-auto w-80 flex-shrink-0">
      ...
      <div className="space-y-3">
        ...
        <div className="grid grid-cols-1 gap-3">
          <button
            onClick={() => handleModelChange(ModelType.FLASH_2_0)}
            disabled={isAnalyzing}
            className={`flex items-start gap-3 p-3 rounded-xl border transition-all text-left ${
              config.model === ModelType.FLASH_2_0
                ? 'bg-primary-500/10 border-primary-500 text-slate-100 ring-1 ring-primary-500/50'
                : 'bg-slate-900 border-slate-700 text-slate-400 hover:border-slate-600'
            }`}
          >
            <Zap className={`w-5 h-5 mt-0.5 ${config.model === ModelType.FLASH_2_0 ? 'text-primary-400' : 'text-slate-500'}`} />
            <div>
              <div className="font-medium">Gemini 2.0 Flash</div>
              <div className="text-xs opacity-70 mt-1">Super-fast multimodal processing. Excellent for general maps.</div>
            </div>
          </button>
          ...
          >
            ...
            <div>
              ...
              <div className="text-xs opacity-70 mt-1">Deep Reasoner. Highest accuracy for complex, messy maps.</div>
            </div>
          </button>
        </div>
      </div>
